<template>
  <section>
    ...
    <p v-if="hasSelectable" class="mb-2 text-[10px] text-slate-500">
      Toca un ejercicio completado para revisarlo o editar su peso
    </p>
...
const props = defineProps<{ items: RoutineItem[]; currentIdx: number; session: RunSession | null; viewIdx: number | null }>()
...
function isSelectable(item: RoutineItem, idx: number): boolean {
  return item.type === 'exercise' && isItemDone(item) && idx !== props.currentIdx
}

const hasSelectable = computed(() => props.items.some((it, i) => isSelectable(it, i)))

function rowClass(item: RoutineItem, idx: number): string {
  if (idx === props.viewIdx) return 'bg-sky-400/10 ring-1 ring-sky-400/30'
  if (idx === props.currentIdx) return 'bg-lime/10'
  if (isSelectable(item, idx)) return 'bg-white/[0.02] cursor-pointer hover:bg-white/5'
  return 'bg-white/[0.02]'
}

function onRowClick(item: RoutineItem, idx: number) {
  if (isSelectable(item, idx)) emit('select', idx)
}
</script>
